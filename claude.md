...
- [x] **Label Multi-Artist Management (Phases F + G)**: Two parallel relationship models for label-on-behalf-of-artist publishing
  - **Phase F — Managed (manager-helps-artist, NIP-26 delegation)**: Non-custodial — artist holds their own nsec; authorises label via a NIP-26 delegation token (signed conditions string). Label includes the token as a `["delegation", artist_pubkey, conditions, signature]` tag on Kind 30050 / Kind 5 events. Artist owns recording rights; fans pay the artist. Renamed to "Manager Authorizations" in UI. Schema columns `node_artists.custody/derivation_index` from migration 003 are reserved (unused) for an alternative custodial model.
  - **Phase G — Signed (label-owns-recording, performer tag)**: Traditional label model — the label IS the publisher and rights-holder. Kind 30050 event signed by the label (event.pubkey = label) with `["p", artist_pubkey, "", "performer"]` attribution tag. No artist signature required; trust + reporting model. **Strict-mode publish gate**: only the artist's current `managed_by` label can publish for them — a second label gets 403 `not_current_label`. Migration 006 adds `node_artists.relationship_type` (self|managed|signed), `cached_tracks.label_pubkey` (consistent who-signed across F+G), and `access_requests.target_relationship_type` (carried through invite-code flow). Operator-only `PATCH /api/label/artists/{pk}` with `managed_by` field handles roster transfers (Magic→Sony, Big Machine→Republic, etc.) — historical tracks signed by previous labels remain in `cached_tracks` as the catalogue history.
  - **Schema migrations**: `005_delegations.sql` (Phase F: `delegation_requests` + `artist_delegations` + `cached_tracks.published_by`), `006_relationship_type.sql` (Phase G: per-artist `relationship_type` + per-track `label_pubkey` + per-invite `target_relationship_type`).
  - **Relay** (Go) — `DelegationStore` (Phase F) + `AdminStore` plumbing `relationship_type` through CRUD (Phase G). `denorm.parseTrack` routes via delegation tag → performer tag → self-publish fallback; populates `label_pubkey` for both label flows. `RedeemInviteCode` writes the invite's `target_relationship_type` to the artist's `node_artists` row.
  - **Orchestrator** — `services/nip26.py` exposes `verify_event_delegation` (Phase F) and `verify_event_performer_attribution` (Phase G). `routers/tracks.py:publish` is a 3-way router (self / managed-delegation / signed-performer) with mutual-exclusion check; strict-mode `managed_by == ctx.pubkey` lookup gates the performer-tag branch. `routers/label.py` extends `add-existing-artist` + `PATCH /artists/{pk}` + `approve` + `invite-codes` with `relationship_type` + operator-only `managed_by` transfer. `routers/access.py:check-invite` previews the type.
  - **Frontend** — `artist-management.html` Relationship column + edit-modal picker + operator-only managed_by transfer + relationship picker on Add Existing Artist; `redeem.html` plain-language preview ("You'll be signed to X" vs "managed by Y"); `releases.html` + `edit-release.html` 3-way `signTrackEvent` router cached per artist; `delegations.html` renamed to "Manager Authorizations". Client SPA `artist.js` uses `CacheAPI.getTracksByArtist()` to catch Phase G performer-tagged tracks; per-release label badge with Kind 0 name resolution.
  - **Verified end-to-end**: operator claims node → label invite → Magic Records onboards → roster invite (signed) → Shibuya Crossings onboards as `signed/managed_by=Magic` → Magic publishes Kind 30050 with `["p", Shibuya, "", "performer"]` tag → `cached_tracks` denorm has `artist_pubkey=Shibuya, label_pubkey=Magic` → Sony Music (second label) attempts publish → 403 `not_current_label` → operator PATCHes Shibuya's `managed_by` to Sony → Sony succeeds, Magic now gets 403, historical Magic tracks remain in catalogue → mixed roster confirmed (Magic also adds a managed Phase F artist).
  - **Caveats**: NIP-07 extension sessions can't sign Phase F delegations (extensions don't expose raw Schnorr); Phase G sidesteps this since the label signs directly. Other Nostr clients that ignore NIP-26 attribute label-signed events to the label, not the artist — Equaliser's own client respects both tags.
  - See [docs/LABEL_DELEGATION_PLEASE_REVIEW.md](docs/LABEL_DELEGATION_PLEASE_REVIEW.md) and [docs/NODE_MANAGEMENT_ARCHITECTURE.md](docs/NODE_MANAGEMENT_ARCHITECTURE.md).
  - **Deferred**: custodial NIP-06 derivation, multi-label simultaneous delegation, delegated profile edits (Kind 0), email notifications, automated royalty splits for 'signed' artists.
...
- [x] **Node Management (Phases A-E)**: Three-tier role architecture (artist, label, node operator)
  ...
  - **Done (Phase C)**: UI role-aware sidebar — `SessionManager.fetchRole()` calls `/api/auth/whoami`, persists `role`/`managedArtists`/`selectedArtistPubkey` in sessionStorage. Sidebar renders role-conditional subtitle, role badge, artist selector dropdown (label/operator with >1 managed artist), and nav groups (`nav-manage` always; `nav-label-admin` for label/operator; `nav-node-admin` for operator only). Artist switches dispatch `equaliser:artist-switched` event for Phase D/E pages to consume. Verified with Playwright across all three roles.
  - **Done (Phase D)**: Label admin pages — `artist-management.html` (table of artists with role/status/fee badges, edit modal for status + fee_model + fee_value, quick suspend/activate; operator view adds Managed By column and shows all artists), `access-requests.html` (Pending/Approved/Declined tabs with counts, approve modal with notes → invite-code-display modal, decline modal with notes), `invite-codes.html` (table of unused codes with source provenance, Generate New for standalone codes, copy buttons throughout). All wired to `/api/label/*` via `SessionManager.authFetch` with NIP-98. Shared `css/admin-base.css` introduced for new pages (existing pages keep their inline styles). Verified with Playwright as both label and operator.
  - **Done (Phase E)**: Operator admin pages — `node-overview.html` (stat tiles for artists/labels/operators/listeners/events/releases/pending requests + service health pings for orchestrator/relay/IPFS/Blossom), `sync-manager.html` (peer relay table from `peer_relays` with status/event/error counts + standard NOSTR relay list + local relay), `ipfs-storage.html` (repo size, storage max, object/pin/swarm-peer counts, peer ID, sample pinned CIDs with gateway view links), `blossom-config.html` (server status + future-card for Phase D mirroring), `user-cache.html` (paginated registered listeners with npub/pubkey/registered/last-seen/enabled), `node-settings.html` (read-only env config grouped into Node/Services/Standard Relays/CORS sections). New backend: relay `GET /api/internal/peer-relays` + orchestrator `/api/operator/sync/peers`, `/ipfs/stats`, `/blossom/status`, `/settings`. All gated on operator role. Verified with Playwright across all 6 pages.
