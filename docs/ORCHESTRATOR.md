...
- **Release Deletion** (implemented): `POST /api/tracks/cleanup` unpins IPFS CIDs and deletes Blossom blobs after client publishes Kind 5 deletion event. Blossom hashes checked for shared references by client before requesting removal. Periodic Blossom orphan cleanup (`blossom_cleanup.py`) removes files from disk that Blossom's DELETE API doesn't clean up. See [DELETE_RELEASES.md](DELETE_RELEASES.md).
