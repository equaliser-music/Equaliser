...
### Label Delegation (NIP-26) — Phase F ✅

This phase is described in detail in [LABEL_DELEGATION_PLEASE_REVIEW.md](LABEL_DELEGATION_PLEASE_REVIEW.md) (the approved plan). Summary of what shipped:

- **Trust model**: artist always holds their own nsec. They authorise a label via a NIP-26 delegation token — a signed conditions string saying "label X may sign Kind N (and other kinds) on my behalf for the next M days". Label includes the token as a `["delegation", artist_pubkey, conditions, signature]` tag on every event published on the artist's behalf. Artist can revoke at any time.
- **Schema**: migration `005_delegations.sql` adds `delegation_requests` (label-initiated requests) and `artist_delegations` (granted token cache, unique on artist+label). Also adds `cached_tracks.published_by` to record the actual signer when a delegation tag is present.
- **Relay backend**: `DelegationStore` (Go) for CRUD + atomic grant; 7 internal endpoints; `denorm.parseTrack` honours the delegation tag — verified events route to the delegator's catalogue, with the signer recorded in `published_by`.
- **Orchestrator backend**: `services/nip26.py` (BIP-340 Schnorr verifier + conditions parser); `routers/delegations.py` (request/grant/decline/incoming/active/revoke); `tracks.publish` accepts label-signed Kind 30050s with valid delegation tags (verified locally + cross-checked against the active record so a revoked delegation is rejected).
- **Frontend**: `delegations.html` artist inbox; `artist-management.html` per-row delegation column with "Request" button + "active" badge; `releases.html` automatically splices in the delegation tag when a label is publishing for a managed artist; sidebar "Delegations" link visible on all admin pages.
- **SessionManager helper**: `signDelegation(delegateePubkey, {kinds, since, until})` builds the canonical conditions string + BIP-340 Schnorr signature. nostr-tools' bundle doesn't expose schnorr, so `delegations.html` loads `@noble/curves` from esm.sh as an ES module and assigns to `window.nobleSchnorr` — `signDelegation` waits for it via `noble-schnorr-ready` event.
- **NIP-07 limitation**: extensions don't generally expose raw Schnorr signing of arbitrary messages (only event signing). Artists granting delegations must use an nsec session for the grant step.
- **Trust-model note for users**: a label can publish on the artist's behalf only while a delegation the artist signed is active. Other Nostr clients that ignore NIP-26 will attribute label-signed events to the label, not the artist — Equaliser's own client respects the tag.

### Rights-Per-Recording (Label as Publisher) — Phase G ✅

Phase F's manager-helps-artist model didn't cleanly fit the traditional record-label relationship where the label OWNS the recording rights and gets paid directly. Phase G adds a parallel relationship type to express that. The two coexist — an artist's row carries one `relationship_type` and an artist's catalogue can span both over time (Taylor Swift on Big Machine → Republic → Taylor's Version).

- **Trust model**: opposite of Phase F. The label IS the publisher and rights-holder. Kind 30050 event is signed by the label (`event.pubkey` = label) with a `["p", artist_pubkey, "", "performer"]` attribution tag. No artist signature on the event. The publish gate is server-side: the relay/orchestrator checks that the artist's current `managed_by` on `node_artists` matches the caller — i.e. only the artist's current label can publish for them. A second label attempting publish gets 403 `not_current_label`.
- **Per-recording attribution, not per-artist**: an artist can be `managed_by` Sony today and have been `managed_by` Magic last year. Historical Magic-signed tracks remain in `cached_tracks` (each with its own `label_pubkey`); Sony cannot publish *new* tracks for them, but Magic's old ones don't disappear. Operator `PATCH /api/label/artists/{pk}` with `managed_by` field handles roster transfers.
- **Schema**: migration `006_relationship_type.sql` adds:
  - `node_artists.relationship_type` (`self` | `managed` | `signed`, default `managed`, CHECK enforced) — backfilled to `self` for unmanaged rows and `managed` for rows with a `managed_by`
  - `cached_tracks.label_pubkey` — consistent who-signed column for both Phase F + G, indexed where non-null
  - `access_requests.target_relationship_type` — carried through the invite-code lifecycle so the artist's `node_artists.relationship_type` is set correctly at redeem time
- **Relay backend**: `AdminStore` signatures extended with `relationship_type` through `ListArtists` / `GetArtist` / `UpdateArtist` / `ListAccessRequests` / `GetAccessRequest` / `GetInviteCode` / `ApproveAccessRequest` / `CreateOrphanInviteCode` / `CreateAccessRequest` / `RedeemInviteCode`. `denorm.parseTrack` is now a 3-way router: delegation tag → performer tag → self-publish; `label_pubkey` populated for both label-tag cases.
- **Orchestrator backend**: `services/nip26.py` adds `find_performer_tag` + `verify_event_performer_attribution`. `routers/tracks.py:publish` is a 3-way router with mutual-exclusion check (event can't carry both delegation AND performer tag). Strict-mode `managed_by == ctx.pubkey` lookup gates the performer-tag branch via `relay_admin.get_artist`. `routers/label.py:add-existing-artist` accepts a `relationship_type` body field; `PATCH /artists/{pk}` accepts both `relationship_type` (any label/operator) and `managed_by` (operator-only — used for label switches). `routers/access.py:check-invite` previews the type.
- **Frontend**: `artist-management.html` adds a Relationship column with badge per artist, the edit modal has a relationship picker AND an operator-only `managed_by` transfer field, and the Add Existing Artist modal carries a `relationship_type` picker (Managed/Signed). `redeem.html` previews the relationship in plain language ("You'll be signed to X" vs "managed by Y"). `releases.html` + `edit-release.html` have a 3-way `signTrackEvent` router cached per artist — looks up the artist's `relationship_type` once per page load, then routes self/managed/signed at sign time. Phase F's sidebar entry renamed from "Delegations" to "Manager Authorizations" (file path unchanged).
- **Client SPA**: `client/js/pages/artist.js` now uses `CacheAPI.getTracksByArtist()` (the denormalized `cached_tracks` endpoint) instead of querying raw events by author — this automatically catches Phase G performer-tagged tracks, which have `event.pubkey = label` but get routed under the performer's `artist_pubkey` by `parseTrack`. Per-release label badge renders with Kind 0 name resolution.
- **Strict mode philosophy**: this is "trust + reporting", not cryptographic protection. Equaliser's relay rejects rogue publishes from non-current labels; another node running a different policy could host them. Disputes resolved off-chain (Kind 5 deletion, label DMCA, operator override). The strict-mode check provides the day-to-day guarantee for the platform's own infrastructure.
- **No client SPA query change required**: a refinement from the original plan — the `#p=` filter change discussed in the spec was unnecessary. The relay's denorm parser maps Phase G performer-tagged events to `cached_tracks.artist_pubkey = performer`, so the existing `/api/cache/tracks?artist=PUBKEY` endpoint automatically returns both self-published AND performer-tagged tracks for the artist. The client just needs to use that endpoint (vs raw event queries) and read `label_pubkey` for the badge.
...
8. ~~Phase F: Label Delegation (NIP-26)~~ ✅ Verified with Playwright end-to-end (label invite → label redeem → roster invite → artist redeem → label requests delegation → artist signs/grants client-side → label sees active delegation badge → revocation persists)
9. ~~Phase G: Rights-Per-Recording (label as publisher)~~ ✅ Verified end-to-end via direct API scenarios (operator claims node → label invite → Magic Records onboards → 'signed' roster invite → Shibuya Crossings onboards as signed/managed_by=Magic → Magic publishes Kind 30050 with performer tag → cached_tracks shows artist=Shibuya, label_pubkey=Magic → Sony Music attempts publish → 403 not_current_label → operator PATCHes managed_by to Sony → Sony succeeds, Magic now gets 403, historical Magic tracks remain → mixed roster confirmed with Phase F managed artist coexisting)
